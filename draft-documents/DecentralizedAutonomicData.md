...
All three recovery methods allow for non-interactive recovery. They all depend on a non trival but not onerous mneumonic for rapid recovery but fall back to a physical copy of the hashes or mnuemonic for slower recovery. Rapid recovery using the GitHub.com or FlickR.com depends on the availability of the databases maintained by the corresponding entities. In the GitHub.com case, should one of the selected repositories and all its forks be deleted then the only recourse would be the physical backup. Likewise with the FlickR.com method should one of the selected images be deleted then the only recourse would be the physical backup.

If interactive recovery is acceptable then using threshold secret sharing is likely the best approach. A physical backup should also be created.
