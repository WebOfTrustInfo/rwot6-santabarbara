...
Looking to the future, it would be possible to create a database with indexing designed to be amenable to mnuemonic devices so that the mneumonic load is minimized. The sole purpose of the database would be to support menumonic seed generation much like the existing DiceWare database is designed to support mneumonic pass phrase generation. If this database had over 1 trillion entries then only 3 records would be required to get 128 bits of security. Each entry would be a hash that is indexed by easy to remember mneumonics.
